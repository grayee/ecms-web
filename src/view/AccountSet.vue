      ...
      <Layout>
        <LayoutPanel region="center" style="height:100%" :bodyStyle="{padding:'5px'}">
          <Panel title="查询条件" :collapsible="true" :bodyStyle="{padding:'10px',marginBottom:'5px'}">
            <Form :model="accountSets" :labelWidth="120" labelAlign="right">
              <div style="margin-bottom:10px">
                <Label for="name" align="right">账套编码：</Label>
                <TextBox inputId="accountSetsNo" name="accountSetsNo" v-model="accountSets.accountSetsNo"></TextBox>
                <Label for="c1" align="right">账套名称：</Label>
                <TextBox inputId="accountSetsName" name="accountSetsName" v-model="accountSets.accountSetsName"></TextBox>
              </div>
              <div style="margin-bottom:10px">
                <Label for="n1" align="right">账套简称：</Label>
                <TextBox inputId="shortName" name="shortName" v-model="accountSets.shortName"></TextBox>

                <Label for="d2" align="right">创建日期： </Label>
                <DateBox inputId="d2" format="yyyy-MM-dd" name="createDateFrom"
                         v-model="accountSets.createDateFrom"></DateBox>
                至
                <DateBox inputId="d2" format="yyyy-MM-dd" name="createDateTo" v-model="accountSets.createDateTo"></DateBox>
                <Label/>
  ...
  export default {
    ...
    methods: {
      ...
      loadPage(pageNumber, pageSize, filters) {
        this.loading = true;
        this.$api.accountSets.accountSetsList({
          pageNo: pageNumber,
          pageSize: pageSize,
      ...
      remove() {
        if (this.checkedIds.length > 0) {
          this.$api.accountSets.accountSetsDel(this.checkedIds).then((response) => {
            this.loadPage(this.pageNumber, this.pageSize);
          }).catch(error => {
            console.log("error", error);
          });
        } else {
          this.$messager.alert({title: "提示信息", icon: "warning", msg: "请至少选中一条记录!",borderType: "none",});
        }
      },
      edit() {
        if (this.checkedIds.length === 1) {
          //path来匹配路由，然后通过query来传递参数
          this.$router.push({path: '/org/accountSets/add?id=' + this.checkedIds[0]});
        } else {
          this.$messager.alert({title: "提示信息", icon: "warning", msg: "请至少选中一条记录!",borderType: "none",});
        }
      },
      ...
      toAdd() {
        this.$router.push({path: '/baseSetting/accountSet/add'});
      },
      onRowClick(row) {
