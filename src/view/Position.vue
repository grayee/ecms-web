      ...
      <Layout>
        <LayoutPanel region="center" style="height:100%" :bodyStyle="{padding:'5px'}">
          <Panel title="查询条件" :collapsible="true" :bodyStyle="{padding:'10px',marginBottom:'5px'}">
            <Form :model="position" :labelWidth="120" labelAlign="right">
              <div style="margin-bottom:10px">
                <Label for="name" align="right">岗位编码：</Label>
                <TextBox inputId="positionNo" name="positionNo" v-model="position.positionNo"></TextBox>
                <Label for="c1" align="right">岗位名称：</Label>
                <TextBox inputId="positionName" name="positionName" v-model="position.positionName"></TextBox>
              </div>
              <div style="margin-bottom:10px">
                <Label for="n1" align="right">岗位简称：</Label>
                <TextBox inputId="shortName" name="shortName" v-model="position.shortName"></TextBox>

                <Label for="d2" align="right">创建日期： </Label>
                <DateBox inputId="d2" format="yyyy-MM-dd" name="createDateFrom"
                         v-model="position.createDateFrom"></DateBox>
                至
                <DateBox inputId="d2" format="yyyy-MM-dd" name="createDateTo" v-model="position.createDateTo"></DateBox>
                <Label/>
  ...
  export default {
    ...
    methods: {
      ...
      loadPage(pageNumber, pageSize, filters) {
        this.loading = true;
        this.$api.position.positionList({
          pageNo: pageNumber,
          pageSize: pageSize,
      ...
      remove() {
        if (this.checkedIds.length > 0) {
          this.$api.position.positionDel(this.checkedIds).then((response) => {
            this.loadPage(this.pageNumber, this.pageSize);
          }).catch(error => {
            console.log("error", error);
          });
      ...
      edit() {
        if (this.checkedIds.length === 1) {
          ...
          this.$router.push({path: '/org/position/add?id=' + this.checkedIds[0]});
        } else {
          this.$messager.alert({
            title: "提示信息",
            icon: "warning",
      ...
      toAdd() {
        this.$router.push({path: '/org/position/add'});
      },
      onRowClick(row) {
